...
[![Agent Skills](https://img.shields.io/badge/reviewed_skills-40-e8f15a.svg)](#已审阅技能样本)

帮你找到更放心安装的 Agent Skill。

SkillCheck 会审阅 Agent Skill 仓库，并把结果整理成一张简单评分卡：它做什么、可能接触什么、能不能跨平台使用，以及安装前应该重点检查什么。
...
| SkillCheck 可以做什么 | 状态 |
| --- | --- |
| 浏览 40 个已审阅 Agent Skill | `apps/web` 已上线 |
...
- Agent Skill 正在变成新的“包生态”。
- 大多数目录帮你找到 Skill；SkillCheck 帮你判断能不能放心用。
- 每个条目都有评分、安全等级、分类、好信号标签、风险标签和平台信号。
- 第一批数据来自 40 个真实高信号仓库，不是占位内容。
- 项目足够小，后续可以扩展成 badge、API 或 PR 审查机器人。
...
Star 数为 2026-07-04 附近的近似值。这个列表不是背书，而是 SkillCheck 用来打磨评分维度的第一批样本。
...
- 真正缺失的不是“发现”，而是信任信息：安全、可迁移性、维护状态、示例和证据。
...
这是 MVP。Scanner 是保守的静态检查器。warning 不代表 skill 恶意，只代表安装到 Agent runtime 前应该先审查这些行为。
